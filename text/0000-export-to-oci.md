...
Exporting to [OCI Image Layout](https://github.com/opencontainers/image-spec/blob/main/image-layout.md) will enable new user's workflows in the top of this functionality. For example:
  - Reduce the Lifecycle complexity removing the interaction with the Daemon.
  - Solve the problem of loosing information when the image is saved into the Daemon keeping the image on disk along with the metadata generated by the Lifecycle. The OCI Image can be used as input for other tools to offer more capabilities to the end users.
...
Also, we are proposing to use symbolic links to reference the blobs from the `run-image` this will help to save some hard drive space for the end user.
...
At high level view the proposed solution can be summarize with the following system landscape diagram from the C4 model

![](https://i.imgur.com/y972lTD.png)

Notice that we are relying on the OCI format Specification to expose the data for `Platforms`

The following new input are proposed to be added to these phases
